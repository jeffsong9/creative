...
```{r LDA_no_uni_ternary, eval=T, echo=F, fig.cap="\\label{fig:no_tern}Ternary plot of the LDA output excluding all unicode characters"}
ap_topics %>%
  group_by(term) %>%
  mutate(sum_prob=sum(phi)) %>%
  mutate(per_word_per_topic=phi/sum_prob) %>%
...
```{r LDA_T_ternary, eval=T, echo=F, fig.cap="\\label{fig:trans_tern}Ternary plot of the LDA output after translating all unicode characters"}
ap_topics %>%
  group_by(term) %>%
  mutate(sum_prob=sum(phi)) %>%
  mutate(per_word_per_topic=phi/sum_prob) %>%
