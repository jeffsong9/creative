...
At its core Latent Dirchlet Allocation is a generative statistical model, that identifies posterior probabilities of words belonging to previously unidentified topics, and topics belonging to documents. The underlying model is generally not analytically tractable, but uses a Gibbs sampling approach instead. Here, we are deriving the  posterior distributions involved in more detail.
...
\ts{I would like to keep the below paragram and bullet points for now.}$\\$
...
\ts{data comes with text format. need data preperation.}
...
The results of LDA output for each cases are reported in \autoref{fig:raw_tern}, \autoref{fig:no_tern}, \autoref{fig:trans_tern}. The result of \autoref{fig:raw_tern} show that including emoji information as a raw unicode character not only enriches semantic(?) information, but also makes the peformance of LDA better.

- it aggregates, otherwise separated, n-gram words into a single character.
