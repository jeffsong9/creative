...
Let $w_{mn}$ be the $n^{th}$ word in the $m^{th}$ document. We assume that the topic of $w_{mn}$ is $z_{m}$, a topic associated with document $m$.  Assume $z_m \sim Multinomial(\theta_m)$, where $\theta_m \sim Dirichlet(\alpha)$ for all $m=1, \dots M$ and $\alpha>0$. For a given topic $z_m=k$, we assume that $w_{mn} \sim Multinomial(\phi_k), n=1, \dots, n_m, m=1, \dots, M$, where $\phi_k \sim Dirichlet(\beta_k)$, $k=1, \sim, K$.
...
\item $\theta_m$: The distribution of topics in document $m$\\
$\theta_m \sim Dir(\boldsymbol\alpha)$\\
\quad $\boldsymbol\theta=
\begin{bmatrix}
\theta_1= (\theta_{1,1}, \theta_{1,2}, \cdots, \theta_{1,K})\\
\theta_2= (\theta_{2,1}, \theta_{2,2}, \cdots, \theta_{2,K})\\
\vdots\\
\theta_M
\end{bmatrix}
$
\item $\boldsymbol\beta$: A vector of prior weights for each word in a topic
\item $\phi_z$: The distirbution of words in topic $z$\\
$\phi_z \sim Dir(\boldsymbol\beta)$
