...
These restaurants are listed into 59 different pages as it is shown in Figure \ref{fig2}. Therefore, obtaining url of all 59 pages would be the first step to scrap each individual restaurant reviews. Note that there are 30 restaurants in each page except for the last page. The following image of Figure \ref{fig3} portraits how the html code is written for this web page.
...
The following steps R code using the `rvest` [@rvest] package will scrap 59 url address for all restaurants in Honolulu, Hawaii.
```{r, eval=FALSE, echo=T}
initial.url="https://www.tripadvisor.com/Restaurants-g60982-Honolulu_Oahu_Hawaii.html"
doc=read_html(initial.url)
```

###Step 2: Scrap the last page number of the review pages.
...
###Step 2: Scrapping the title of an individual review and its url.
...
###Step 3: Scrapping the review entry of an individual review.
\begin{figure}[htbp]
\centering
...
The head and the tail of the generated dataset is given in Figure \ref{fig7}.
