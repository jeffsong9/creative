...
Inspite of applying preparation processes such as the cleaning procedure during the Data gathering phase, the following techniques could be put to use in order to enhance the accuracy of the text-mining analysis.
...
```{r remove stop words, eval=F, echo=T}
...
An example result of using the above code is as follows.
```{r, echo=T}
eg="This is an example of removing the stop words."
eg_vdc=VectorSource(eg)
eg_my.corpus = Corpus(eg_vdc)
eg_my.corpus = tm_map(eg_my.corpus, removeWords, c("the", stopwords("english"))) 
strwrap(eg_my.corpus[[1]])
```

Note that stop words such as "is", "an", "the", "of" are removed from the given sentence.

##Stemming

Due to structural and grammatical reasons of English, a family of words that are driven from a single root word is used in different forms. For example, words such as "stems", "stemmer", "stemming", and "stemmed" are all based on a root word "stem". These different form of words with the same root word may cause difficulties applying some text mining techniques. Therefore, stemming procedure should be introduced to reduce inflectional forms of a word to a word in its root form. The `tm` package is again used for the stemming process and its code is given as the following.
...
```{r, echo=T}
eg="This is an example of stemming process where words are stemmed into its root form."
eg_vdc=VectorSource(eg)
eg_my.corpus = Corpus(eg_vdc)
eg_my.corpus = tm_map(eg_my.corpus, stemDocument, language="english")
strwrap(eg_my.corpus[[1]])
```

Note that words "exmple", "stemming", "stemmed", "words", and "its"  were reduced to "exampl", "stem"", "word", and "it".

##Stop words and Stemming procedure combined

A simple function as below could combine the two processes above together.
...
```{r, echo=T}
StopNStem("This is an example of removing the stop words. This is an example of stemming process where words are stemmed into its root form.")
```

```{r, echo=T, eval=F}
setwd("C:/Users/Taikgun Song/Desktop/CC Meeting/CC")
Final=read.csv("data/Final.csv", stringsAsFactors = F)
ea.review.SWS=lapply(Final$ea.review, StopNStem) %>% do.call("rbind",.) %>% rbind.data.frame(stringsAsFactors=F)
View(ea.review.SWS)
write.csv(ea.review.SWS, "ea_review_SWS.csv", row.names = F)
```
